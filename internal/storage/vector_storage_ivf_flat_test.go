...
func TestVectorEngineImpl_InsertAndSearch_IVF32Flat(t *testing.T) {
	...
	t.Run("Remove vector", func(t *testing.T) {
		...
		cleanDataPath := "testdata/vector_data_ivf_remove.db"
		cleanIndexPath := "testdata/vector_index_ivf_remove.faiss"
		cleanWalPath := "testdata/vector_wal_ivf_remove.db"
		...
		// Insert multiple vectors to ensure training (IVF32 needs at least 32 vectors)
		for i := 0; i < 100; i++ {
			vec := randomVector(maxVectorSize)
	...
	t.Run("Remove non-existent vector", func(t *testing.T) {
		// Create a fresh engine for this test
		cleanDataPath := "testdata/vector_data_ivf_remove_nonexistent.db"
		cleanIndexPath := "testdata/vector_index_ivf_remove_nonexistent.faiss"
		cleanWalPath := "testdata/vector_wal_ivf_remove_nonexistent.db"
