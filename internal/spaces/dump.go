...
func collectFlatMetaDataPaths(spacePath string) ([]string, error) {
	manifestPath := filepath.Join(spacePath, "flat_meta_data_segments.manifest.json")
	if data, err := os.ReadFile(manifestPath); err == nil {
		var manifest struct {
			Segments []struct {
				DataFile string `json:"data_file"`
	...
	for dec.More() {
		...
		case "space_meta":
			...
			if err := json.Unmarshal(raw, &sm); err != nil {
				return stats, fmt.Errorf("parse space_meta record: %w", err)
			}
		...
		case "kv":
			...
			if err := json.Unmarshal(raw, &rec); err != nil {
				return stats, fmt.Errorf("parse kv record: %w", err)
			}
		...
		case "vector":
			...
			if err := json.Unmarshal(raw, &rec); err != nil {
				return stats, fmt.Errorf("parse vector record: %w", err)
			}
...
func restoreKeyValueData(spacePath string, meta spaceMeta, data map[string]string, mode string) (int, error) {
	...
	if mode == "merge" {
		...
		if _, err := os.Stat(dataPath); err == nil {
			if err := scanKVDataFile(dataPath, existing, existingDel); err != nil {
				return 0, fmt.Errorf("scan existing data: %w", err)
			}
		}
...
func restoreFlatMetaVectorData(spacePath string, meta spaceMeta, data []DumpVectorRecord, mode string) (int, error) {
	...
	cleanupAuxFiles(spacePath, []string{
		"flat_meta_wal.db",
		"flat_meta_data_segments.manifest.json",
	})
